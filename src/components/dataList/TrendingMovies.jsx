...
import { fetchTrendingMovies } from '../../features/contentSlice';
...
const TrendingMovies = () => {
    ...
    const { trendingMovies, trendingMoviesLoading, trendingMoviesError, trendingMoviesSuccess } = useSelector((state) => state.content)
        ...
        <>
            <div className="container-fluid px-md-4">
                ...
                <div className="movie-container d-flex">
                    {trendingMoviesLoading && <Loader />}
                    {trendingMoviesError && <Loader />}
                    {trendingMoviesSuccess && trendingMovies.map((movie, index) => <div className="card movie-card" key={index}><Link to="movie" onClick={(e) => handleTap(e, movie)}><img src={movie.poster_path ? `https://image.tmdb.org/t/p/w500/${movie.poster_path}` : `https://sushihousemenu.com/Media/Default/Menu%20Items/placeholder.png`} alt="moviecard_image" /></Link>
                        ...
                        </div><span className="badge bg-primary position-absolute" style={{top: 10, left: 10}}>{movie.vote_average.toFixed(1)}</span><button className="btn text-light position-absolute" type="button" style={{right: 10}}><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none" style={{marginBottom: 8}}>
